...
- `clipper.cpp/.hpp`：Angus Johnson Clipper 6.4.2，Boost Software License 1.0，全文见 `../../../resource/license/BOOST-1.0.txt`

相对封装仓库基准版本，本项目增加 `OCR_STATIC` 导出宏分支，由顶层 CMake 直接编译为静态库，并修正 MSVC 报告的整数宽度与格式化警告。`workflow` 还初始化了 ONNX session 指针、让 Windows 模型与字典路径按 UTF-8 解码，并让字典打开失败通过异常上报；相关修改文件顶部带有显著说明。OCR 模型放在 `resource/model`，不会混入 C++ 源码目录。
